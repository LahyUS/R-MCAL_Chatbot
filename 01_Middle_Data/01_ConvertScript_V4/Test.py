...
def Doc2MiddleData(Document):
    ...
    with open(docname, 'rb') as pdf_file:
        ...
        for i in range(num_pages):
            count =0
            output_file_path = os.path.join("/home/banvien/RVC/Workspace/", f"output.pdf")
            output_pdf = PdfWriter()
            output_pdf.add_page(pdf_reader.pages[i]) 
            with open(output_file_path, 'wb') as output_file:
                output_pdf.write(output_file)
            doc = aw.Document(output_file_path)
            doc.save("/home/banvien/RVC/Workspace/output.html")
           # read the text from the document  
            with open("/home/banvien/RVC/Workspace/output.html", 'r') as file:
